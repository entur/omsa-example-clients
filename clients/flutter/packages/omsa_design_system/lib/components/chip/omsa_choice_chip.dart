...
/// Choice chip that behaves like a radio button within a group.
class OmsaChoiceChip extends StatefulWidget {
  const OmsaChoiceChip({
    super.key,
    required this.label,
    required this.value,
    this.groupValue,
  ...
  @override
  State<OmsaChoiceChip> createState() => _OmsaChoiceChipState();
}

class _OmsaChoiceChipState extends State<OmsaChoiceChip> {
  ...
  Color _resolveTextColor(BuildContext context, OmsaChipColors colors) {
    if (_isDisabled) {
    ...
    if (_isSelected) {
      if (_isHovered && widget.mode == OmsaComponentMode.contrast) {
        return colors.textUnselected;
      }
      return colors.textSelected;
    }

    if (_isPressed && widget.mode == OmsaComponentMode.contrast) {
      return colors.textSelected;
    }
  ...
  Color _resolveIconColor(BuildContext context, OmsaChipColors colors) {
    if (_isDisabled) {
      return colors.iconDisabled;
    }

    if (_isSelected) {
      final bool hoverOverrides =
          _isHovered && widget.mode == OmsaComponentMode.contrast;
      return hoverOverrides ? colors.iconUnselected : colors.iconSelected;
    }

    return colors.iconUnselected;
  }
  ...
  void _handleTap() {
    if (_isDisabled || _isSelected || widget.onSelected == null) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final Color textColor = _resolveTextColor(context, colors);
    final Color iconColor = _resolveIconColor(context, colors);
